...
namespace nsyshid::backend::emulated
{
	BackendEmulated::BackendEmulated()
		: m_dimensionsNetworkListener(std::make_unique<DimensionsNetworkListener>())
	{
	...
	BackendEmulated::~BackendEmulated()
	{
		m_dimensionsNetworkListener->Stop();
	}
	...
	void BackendEmulated::AttachVisibleDevices()
	{
		...
		if (GetConfig().emulated_usb_devices.emulate_dimensions_toypad && !FindDeviceById(0x0E6F, 0x0241))
		{
			...
			if (AttachDevice(device))
				m_dimensionsNetworkListener->Start(static_cast<uint16>(GetConfig().emulated_usb_devices.dimensions_toypad_listener_port.GetValue()));
		}
	}
} // namespace nsyshid::backend::emulated
