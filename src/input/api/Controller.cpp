...
namespace
{
	...
	bool IsToypadPickerInputActive()
	{
		HANDLE event = OpenEventW(SYNCHRONIZE, FALSE, kToypadPickerInputEvent);
		if (!event)
			return false;
...
{
	// The standalone Toypad picker owns controller input while its window is active.
	// XInput is otherwise shared between processes, so return a neutral state until
	// the picker releases the named event on focus loss or minimization.
	if (IsToypadPickerInputActive())
	{
		m_last_state = {};
		return m_last_state;
	}
